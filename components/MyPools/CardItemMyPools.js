...
function CardItemMyPools(props) {
  const { classes, item, userAddress, intl } = props;
  return (
    <Card className={classes.root}>
      <CardContent>
        <div
          role="button"
          onClick={() => Router.pushRoute(`/pools/${item.address}`)}
        >
          <Typography variant="subheading">
            ...
            {userAddress === item.manager && (
              <span style={styleSpan}>
                {' '}
                {intl.formatMessage(messages.youAreAdminMyPool)}
              </span>
            )}
          </Typography>
          <br />
          <Typography variant="subheading">
            <span>
              {intl.formatMessage(messages.totalPlayerMyPool)}{' '}
              {item.totalPlayers} &nbsp;&nbsp;{' '}
              {intl.formatMessage(messages.prizeMyPool)} {item.totalBalance} ETH{' '}
            </span>
          </Typography>
        </div>
      </CardContent>
