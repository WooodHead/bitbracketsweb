...
const messages = defineMessages({
  shortDescription: {
    id: 'shortDescription',
    defaultMessage:
      'The platform for creating prediction pools for sporting events.',
    description: ''
  },
  description: {
    id: 'description',
    defaultMessage:
      'BitBrackets is a smart contract driven platform for creating prediction pools for sporting events based on Ethereum blockchain.',
    description: ''
  }
});
...
const defaultImage =
  'http://res.cloudinary.com/bitbrackets/image/upload/v1527219334/home_bitbrackets.png';
const defaultFavicon = '/static/favicon.ico';

const Header = props => {
  const { title, description, favicon, url, image, intl } = props;
  let currentTitle =
    bitbrackets + ' | ' + intl.formatMessage(messages.shortDescription);
