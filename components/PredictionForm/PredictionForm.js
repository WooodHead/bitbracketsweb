...
class PredictionForm extends Component {
  ...
  renderTabContent() {
    ...
    const { classes, matches, groups, predictions, update, read } = this.props;

    return (
      <Grid container className={classes.matches} spacing={16}>
        {_.filter(matches, match => match.data.group === groups[activeTab]).map(
          match => (
            <Grid item xs={12} sm={6} key={match.index}>
              <MatchCard
                match={match}
                prediction={
                  predictions[match.index]
                    ? predictions[match.index].prediction
                    : null
                }
                update={update}
                read={read}
      ...
      <Paper className={classes.form}>
        ...
        >
          {groups.map(group => (
            <Tab
              key={group}
              label={`${intl.formatMessage(messages.group)}- ${group}`}
            />
          ))}
        </Tabs>
      </Paper>
